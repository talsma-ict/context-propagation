 ...
 */
public class SpringSecurity6ContextManager implements ClearableContextManager<Authentication> {
    ...
    private static final class AuthenticationContext implements Context<Authentication> {
        ...
        private AuthenticationContext(SecurityContext current, SecurityContext previous, boolean alreadyClosed) {
            ...
            ContextManagers.onActivate(SpringSecurity6ContextManager.class, auth(current), auth(previous));
        }
        ...
        public void close() {
            if (closed.compareAndSet(false, true)) {
                ...
                ContextManagers.onDeactivate(SpringSecurity6ContextManager.class, auth(current), auth(previous));
            }
        }
